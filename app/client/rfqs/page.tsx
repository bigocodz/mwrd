...
export default function ClientRfqsPage() {
  ...
  return (
    <>
      <PageHeader
        title="My RFQs"
        description="Track all your procurement requests and their quote status."
        actions={
          <Button asChild>
            <Link href="/client/rfqs/create">
      ...
      <Panel>
        <div className="mb-5 flex items-center gap-3">
          <Select value={filter} onChange={(e) => { setFilter(e.target.value); setPage(1) }} className="w-[160px]">
        ...
        {loading ? (
          ...
          <EmptyMessage>No RFQs yet. Create your first request!</EmptyMessage>
        ) : (
          <>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {paged.map((r) => (
                  <TableRow key={r.id} onClick={() => router.push(`/client/rfqs/${r.id}`)}>
                    ...
                    <TableCell>
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); router.push(`/client/rfqs/${r.id}`) }}
            ...
            <PaginationControls page={page} totalPages={numPages} total={rfqs.length} onPageChange={setPage} className="mt-4" />
          </>
        )}
      </Panel>
    </>
