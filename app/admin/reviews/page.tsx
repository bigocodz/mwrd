...
export default function AdminReviewsPage() {
  ...
  return (
    <>
      <PageHeader title="Reviews" description="Supplier reviews submitted by clients." />
      <Panel>
        {loading ? (
              ...
              <TableBody>
                {paged.map((item: any) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="font-medium text-gray-900">{item.rating ?? '—'} / 5</TableCell>
                    <TableCell className="max-w-xs truncate text-gray-600">
                      {item.comment ? item.comment.slice(0, 60) + (item.comment.length > 60 ? '…' : '') : '—'}
                    </TableCell>
