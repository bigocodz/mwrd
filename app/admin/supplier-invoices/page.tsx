...
function SupplierInvoiceStatusBadge({ status }: { status: string }) {
  const variants: Record<string, 'warning' | 'success' | 'default'> = {
    PENDING: 'warning',
    PAID: 'success',
  }
...
export default function AdminSupplierInvoicesPage() {
  ...
  return (
    <>
      <PageHeader title="Supplier Invoices" description="Invoices submitted by suppliers for fulfilled orders." />
      <Panel>
        ...
        {loading ? (
          ...
          <EmptyMessage>No supplier invoices found.</EmptyMessage>
        ) : (
          <>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {paged.map((item: any) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell><SupplierInvoiceStatusBadge status={item.status} /></TableCell>
