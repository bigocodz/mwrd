...
function ProductRequestStatusBadge({ status }: { status: string }) {
...
export default function AdminProductRequestsPage() {
  ...
  useEffect(() => {
    ...
    let q = (supabase as any)
      .from('product_addition_requests')
      .select('*')
  ...
  return (
    <>
      <PageHeader
        ...
        description="Supplier requests to add new products to the catalog."
      />
      <Panel>
        ...
        {loading ? (
          ...
          <EmptyMessage>No product requests found.</EmptyMessage>
        ) : (
          <>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {paged.map((item: any) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell><ProductRequestStatusBadge status={item.status} /></TableCell>
